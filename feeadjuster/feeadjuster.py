...
def get_ratio_hard(our_percentage):
    ...
    Between 0 and 0.5 is exponential and between 0.5 and 1 is linear
    """
    if our_percentage > 0.5:
        return (1 - our_percentage) * 2
    return 400**(0.5 - our_percentage)


def get_chan2(plugin: Plugin, scid: str):
...
def maybe_setchannelfee(plugin: Plugin, scid: str, basefee = None, ppmfee = None):
    if basefee == None:
        basefee = plugin.adj_basefee
    if ppmfee == None:
        ppmfee = plugin.adj_ppmfee
    chan = get_chan2(plugin, scid)
    if chan is None:
        return False
    if basefee == chan["base_fee_millisatoshi"] and ppmfee == chan["fee_per_millionth"]:
        return False
    try:
        plugin.rpc.setchannelfee(scid, basefee, ppmfee)
...
plugin.add_option(
    ...
    "Can be 'default' or 'hard' for higher difference"
    "string"
)
plugin.add_option(
